...
<T>(
  title: string,
  initialValue: T,
  props: StandardEditorProps,
  applyChanges: ApplySEModifications<T>,
  backUrl?: string,
  footer: boolean = false,
  iconButtons: boolean = false): StandardEditorState<T> {
  // const {match: {path}} = props;
  const path = useNavigate();
  const editable = path.name.indexOf('/edit') !== -1 || path.name.indexOf('/new') !== -1;
  const isNew = path.name.indexOf('/new') !== -1;
...
export function useOnNavigation<T>(standardEditor: StandardEditorState<T>) {
  const {
    setRedirectUrl, isNew, setInitialValues, onNewForm, onLoadForm, editable,
