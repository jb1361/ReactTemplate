...
export function EditorForm<T extends object>({standardEditor, schema, children, renderAfterForm, touchFormOnLoad, validateOnMount, validate}: Props<T>) {
  const {
    ...
    redirectUrl, title, backUrl, onSave, loading, criticalErrorMessage, performFormTouch,
    setPerformFormTouch, performFormReset, setPerformFormReset, onFormReset,
    onClone, onCloneUrl, setRedirectUrl, newUrl, onDelete, isDeleting, setIsDeleting, footer, iconButtons
  } = standardEditor;
  if (loading) {
        ...
        return (
          <>
            <Form onSubmit={handleSubmit}>
              ...
              <div style={iconButtons ? {display: 'flex'} : {}}>
                {children(formik)}
                {footer ?  (
                  <FormFooter
                    title={title}
